...
void init_lap_window() {
    window_init(&laps_window, "Lap times");
    window_set_background_color(&laps_window, GColorWhite);
    window_set_window_handlers(&laps_window, (WindowHandlers){
        .appear = (WindowHandler)handle_appear
    });

    scroll_layer_init(&scroll_view, GRect(0, 0, 144, 152));
    scroll_layer_set_click_config_onto_window(&scroll_view, &laps_window);

    GFont laps_font = fonts_get_system_font(FONT_KEY_GOTHIC_18);
    ...
    layer_add_child(window_get_root_layer(&laps_window), &scroll_view.layer);
    ...
    layer_add_child(window_get_root_layer(&laps_window), &no_laps_note.layer);
}

void show_laps() {
    window_stack_push(&laps_window, true);
}
