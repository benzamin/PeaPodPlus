...
#include "laps_stopwatch.h"
#include "common.h"


static Window stopwatchWindow;
//AppContextRef app;
...
static void window_unload_st(Window* window);
static void window_load_st(Window* window);
static bool is_stopwatch_shown = false;


void init_stopwatch_window() {
	
	window_init(&stopwatchWindow, "Stopwatch");
    window_set_window_handlers(&stopwatchWindow, (WindowHandlers){
        .unload = window_unload_st,
        .appear = window_load_st
    });
	
    window_stack_push(&stopwatchWindow, true);
	window_set_background_color(&stopwatchWindow, GColorBlack);
    window_set_fullscreen(&stopwatchWindow, false);
...
static void window_load_st(Window* window)
{
    ...
    GFont seconds_font = fonts_get_system_font(FONT_KEY_GOTHIC_18);
    ...
    init_lap_window();
	is_stopwatch_shown = true;
}

static void window_unload_st(Window* window)
{
    bmp_deinit_container(&button_labels);
	is_stopwatch_shown = false;
}
...
void stop_stopwatch() {
    ...
    if(update_timer != APP_TIMER_INVALID_HANDLE) {
        if(app_timer_cancel_event(g_app_context, update_timer)) {
...
void start_stopwatch() {
    ...
    last_pebble_time = 0;
    start_time = 0;
    update_timer = app_timer_send_event(g_app_context, 100, TIMER_UPDATE);
}
...
void stopwatch_handle_timer(AppContextRef ctx, AppTimerHandle handle, uint32_t cookie) {
    if(!is_stopwatch_shown)
		return;
